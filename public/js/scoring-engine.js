...
(function (exports) {
  ...
  function ensureTeamPlayer(id, team) {
    return isPlayerInTeam(id, team) ? id || null : null;
  }
      ...
      players: (team.players || []).map(p => {
        ...
        if (!isBat && p.id === ball.fielderId) {
          np = {
            ...
            fieldingStats: {
              ...np.fieldingStats,
              catches: (np.fieldingStats?.catches || 0) + (ball.wicketType === 'CAUGHT' ? 1 : 0),
              runOuts: (np.fieldingStats?.runOuts || 0) + (ball.wicketType === 'RUN_OUT' ? 1 : 0),
              stumpings: (np.fieldingStats?.stumpings || 0) + (ball.wicketType === 'STUMPED' ? 1 : 0),
              droppedCatches: (np.fieldingStats?.droppedCatches || 0) + (ball.isDroppedCatch ? 1 : 0)
            }
          };
        }
  ...
  function recalculateMatch(match) {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      const isBattingA = isTeamA(current.battingTeamId, current);
      ...
      if (!ball.isAdjustment) {
        ...
        if (!sId && ball.strikerId && ball.strikerId !== victimId && !isPlayerUnavailable(ball.strikerId, current)) sId = ball.strikerId;
        if (!nsId && ball.nonStrikerId && ball.nonStrikerId !== victimId && !isPlayerUnavailable(ball.nonStrikerId, current)) nsId = ball.nonStrikerId;
      ...
      if (current.currentInnings === 1 && inningsEnded) {
        ...
        const presetStrikerId = carrySecondInningsSelections ? ensureTeamPlayer(healLegacyId(match.strikerId, nextBattingTeam), nextBattingTeam) : null;
        const presetNonStrikerId = carrySecondInningsSelections ? ensureTeamPlayer(healLegacyId(match.nonStrikerId, nextBattingTeam), nextBattingTeam) : null;
        const presetBowlerId = carrySecondInningsSelections ? ensureTeamPlayer(healLegacyId(match.currentBowlerId, nextBowlingTeam), nextBowlingTeam) : null;
        const presetLastBowlerId = carrySecondInningsSelections ? ensureTeamPlayer(healLegacyId(match.lastBowlerId, nextBowlingTeam), nextBowlingTeam) : null;
        ...
        current = {
          ...
          pendingAction: match.isSecondInningsStarted ? 'NONE' : 'START_SECOND_INNINGS'
        };
        ballsInOver = 0;
    ...
    if (current.status === 'LIVE') {
      ...
      const batTeam = isTeamA(current.battingTeamId, current) ? current.teamA : current.teamB;
      const squadSize = current.gullyRules?.unequalTeams
      ...
      if (current.currentInnings === 1 && inningsEnded) {
        ...
        current = {
          ...
          wicketHistory: [],
          strikerId: presetStrikerId,
          nonStrikerId: !Boolean(current.gullyRules?.singleSideBatting) && presetNonStrikerId && presetNonStrikerId !== presetStrikerId ? presetNonStrikerId : null,
          currentBowlerId: presetBowlerId,
          lastBowlerId: presetLastBowlerId,
          pendingAction: match.isSecondInningsStarted ? 'NONE' : 'START_SECOND_INNINGS'
        };
  ...
  function calculateInningsStats(balls, options) {
