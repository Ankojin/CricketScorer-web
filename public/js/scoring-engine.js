...
class ScoringEngine {
    ...
    static recalculateMatchFromHistory(match) {
        ...
        if (current.status === 'LIVE') {
            ...
            const activeStrikerId = current.strikerId ||
            ...
            const activeNonStrikerRaw = current.nonStrikerId ||
                (explicitNonStrikerId && !this.isPlayerUnavailable(explicitNonStrikerId, current) ? explicitNonStrikerId : null);
            const activeNonStrikerId = !Boolean(current.gullyRules?.singleSideBatting) &&
                activeNonStrikerRaw &&
    ...
    static calculateInningsStats(balls, options) {
            ...
            players: (team.players || []).map(p => {
                ...
                if (!isBat && p.id === ball.fielderId) {
                    np = {
                        ...
                        fieldingStats: {
                            ...np.fieldingStats,
                            catches: np.fieldingStats.catches +
                                (ball.wicketType === 'CAUGHT' ? 1 : 0),
                            runOuts: np.fieldingStats.runOuts +
                                (ball.wicketType === 'RUN_OUT' ? 1 : 0),
                            stumpings: np.fieldingStats.stumpings +
                                (ball.wicketType === 'STUMPED' ? 1 : 0),
                            droppedCatches: np.fieldingStats.droppedCatches + (ball.isDroppedCatch ? 1 : 0)
                        }
                    };
                }
                return np;
